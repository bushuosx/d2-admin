<template>
  <el-card style="margin:4px 0px">
    ...
    <input @change="handleChange" ref="selectfiledialog" type="file" hidden/>
    <div style="margin:2px" class="fileselector" v-for="(item,index) in uploadfiles" v-bind:item="item" v-bind:key="item.key" v-bind:index="index">
      ...
      <el-button style="margin:auto auto auto 4px" size="mini" type="danger" @click="remove(index)" circle>
  ...
  methods: {
    ...
    handleChange () {
      ...
      if (file) {
        ...
        Message({ message: '已取消上传', type: 'warning' })
      }
    },
    hashfile (file, key) {
    ...
    upload (key) {
      ...
      for (let i in this.uploadfiles) {
        if (this.uploadfiles[i].key === key) {
          ...
          if (uploadfile.status !== 1 && uploadfile.status !== 4) {
            Message({ message: '文件此时不能上传', type: 'error' })
            return
          }
          break // 退出for key
        }
      ...
      let emitChange = this.emitFileChange
    ...
    remove (index) {
      let changed = false
      if (this.uploadfiles[index].status === 3) {
        changed = true
      }
    ...
    emitFileChange () {
      this.$emit('file-changed', this.getFileIdList())
    }
  }
}
</script>
