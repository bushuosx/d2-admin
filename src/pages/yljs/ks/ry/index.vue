...
export default {
  ...
  computed: {
    ...
    isKSRoleManager () {
      return !!this.user && this.user.hasAnyPermission([this.user.Permissions.科室角色管理权限], this.ksid)
    }
  },
  data () {
    return {
  ...
  methods: {
    ...
    handleBan (ry) {
      if (ry) {
        if (ry.id === this.user.id) {
          // this.$message.warning('不能移除自己')
          // return
          this.$confirm('你将要从科室中称除自己, 是否继续?', '提示', {
            ...
            .then(() => {
              this.$prompt('此操作将从本科室中移除人员，若该人员有科室管理权限，将一并移除。请在下方输入操作原因：', '提示', {
                ...
                this.loading = true
                ryksapi.ban(ry.id, this.ksid, value).then(res => {
                  this.loading = false
                  if (res.code === 1) {
