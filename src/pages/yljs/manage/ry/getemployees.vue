<template>
  <d2-container v-loading="loading">
    ...
    <el-table :data="rylist"
      ...
      <el-table-column prop="ksCode"
        label="科室编码">
      </el-table-column>
      <el-table-column prop="ksManager"
        label="科室管理员">
      </el-table-column>
      <el-table-column prop="yyManager"
        label="医院科室员">
      </el-table-column>
      <el-table-column label="操作">
        <template slot-scope="scope">
          <template v-if="showAction(scope.row)">
            <el-button size="mini"
              @click="showzl(scope.row)">查看资料</el-button>
            <el-button size="mini"
              @click="showryjs(scope.row)">查看技术信息</el-button>
          </template>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    showryjs (row) {
      this.$router.push({ name: 'yljs-ryjs-listbyry', params: { ryid: row.ryid } })
    },
    showzl (row) {
      this.$router.push({ name: 'yljs-ryprofile-index', params: { ryid: row.ryid } })
    }
  }
}
</script>
