<template>
    <d2-container v-loading="loading">
        <div slot="header">
            <strong>导入科室信息</strong>
        </div>
              <div>数据量较大时，操作耗时将较长。默认等待 <strong>20</strong> 秒。</div>
        <div>
            <ol>
        ...
        <div v-if="faildList">
            ...
            <span v-for="(item,index) in faildList" :key="index">{{item.bm}}；</span>
        </div>
    </d2-container>
</template>
...
export default {
  ...
  methods: {
    ...
    handleSubmit () {
      ...
      let rst = sd.map(v => { return { MC: v[mcHeader], BM: v[bmHeader] } })

      this.loading = true
      ksapi.importks(rst).then(res => {
        this.loading = false
        if (res.code === 1) {
